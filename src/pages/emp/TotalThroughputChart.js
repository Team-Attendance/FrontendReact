...
export default function TotalThroughputChart({ documentStatusData }) {
  ...
  const totalDisposeCount = leave_dispose_count + odd_biz_dispose_count;
  const totalWaitingCount = leave_waiting_count + odd_biz_waiting_count;
      ...
      data:
        totalDisposeCount === 0 ?
          [
            ...
            { value: totalWaitingCount, name: '미처리', itemStyle: { color: '#C9CBCF ' } },
          ]

          :
          totalWaitingCount === 0 || (totalWaitingCount === 0 && totalDisposeCount === 0) ?
            [
              { value: totalDisposeCount, name: '처리', itemStyle: { color: '#36A2EB' } },
    ...
    <div style={{ height: '100%', position: 'relative' }}>
      <div style={{ width: '100%', height: '100%', position: 'absolute', display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: 'bold', lineHeight: '20px', textAlign: 'center' }}>
        <p style={{ paddingBottom: '10px' }}>
          ...
          {totalWaitingCount + totalDisposeCount === 0 ?
            <span style={{ fontSize: '1rem' }}>0%</span>
            : <span style={{ fontSize: '1rem' }}>{Math.round(((totalDisposeCount / (totalWaitingCount + totalDisposeCount)) * 100) * 10) / 10}%</span>}
        </p>
      </div>
